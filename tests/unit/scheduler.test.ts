import { scheduleOrder } from '../../src/scheduler/just_in_time_scheduler';
import { Part } from '../../src/domain/types';

describe('scheduler', () => {
  const wcA = 'WCA';
  const wcB = 'WCB';

  const partA: Part = { id: 'A', name: 'A', leadTimeMins: 30, workcenterId: wcA };
  const partB: Part = { id: 'B', name: 'B', leadTimeMins: 60, workcenterId: wcB };

  test('aligns completion times for different workcenters', () => {
    const base = new Date('2025-01-01T00:00:00Z');
    const res = scheduleOrder({ orderId: 'O1', parts: [partA, partB], bom: [
      { partId: 'A', quantity: 2 },
      { partId: 'B', quantity: 1 },
    ...
    const perPartMaxEnd = new Map<string, number>();
    for (const u of res.units) {
      const end = u.endTime.getTime();
      const prev = perPartMaxEnd.get(u.partId) ?? -Infinity;
      if (end > prev) perPartMaxEnd.set(u.partId, end);
    }
  ...
  test('serializes units on same workcenter', () => {
    ...
    const base = new Date('2025-01-01T00:00:00Z');
    const res = scheduleOrder({ orderId: 'O2', parts, bom: [
      { partId: 'A', quantity: 2 },
      { partId: 'B', quantity: 1 },
    ], baseStart: base, slackTolerancePercent: 0 }); // Disable slack for deterministic test
  ...
  test('handles very long lead time part as bottleneck', () => {
    ...
    const base = new Date('2025-01-01T00:00:00Z');
    const res = scheduleOrder({ orderId: 'O3', parts, bom: [
      { partId: 'A', quantity: 1 },
      { partId: 'C', quantity: 1 },
  ...
  test('applies slack tolerance correctly', () => {
    ...
    const base = new Date('2025-01-01T00:00:00Z');
    const res = scheduleOrder({ orderId: 'O4', parts, bom: [
      { partId: 'A', quantity: 1 },
      { partId: 'B', quantity: 1 },
  ...
  test('handles setup times between different parts', () => {
    ...
    const base = new Date('2025-01-01T00:00:00Z');
    const res = scheduleOrder({ orderId: 'O5', parts, bom: [
      { partId: 'A', quantity: 1 },
      { partId: 'B', quantity: 1 },
    ], baseStart: base, slackTolerancePercent: 0 });
