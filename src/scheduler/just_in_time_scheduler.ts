import { Part, ScheduledUnit, ScheduleResult } from '../domain/types';

export interface ScheduleInput {
  orderId: string;
  parts: Part[];
  bom: { partId: string; quantity: number }[];
  // baseStart: earliest time production can start
  baseStart?: Date;
  // targetCompletion: desired completion time for final integration; defaults to soonest possible based on bottleneck
  targetCompletion?: Date;
  slackTolerancePercent?: number; // ±5% alignment window
...
export function scheduleOrder(input: ScheduleInput): ScheduleResult {
  const now = input.baseStart ?? new Date();
  const slackTolerance = input.slackTolerancePercent ?? 5;
  const partById = new Map(input.parts.map(p => [p.id, p] as const));

  // Group BOM by workcenter
  const workcenterToPartOrders = new Map<string, { part: Part; quantity: number }[]>();
  for (const item of input.bom) {
    const part = partById.get(item.partId);
    if (!part) throw new Error(`Unknown part ${item.partId}`);
